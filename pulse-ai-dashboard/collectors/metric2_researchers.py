...
class Metric2Collector(BaseCollector):
    ...
    def collect(self, year: int) -> MetricSeries:
        ...
        series = MetricSeries(
            metric_type=MetricType.RESEARCHERS,
            name=self.config["metric_2"]["name"],
            description=self.config["metric_2"]["description"],
            unit=self.config["metric_2"].get("unit", "чел."),
        )

        data = self.manual_data.get("metric_2_researchers", {})
        ...
        if year_data:
            ...
            logger.info(f"Метрика 2 ({year}): {value} исследователей")
        else:
            logger.warning(f"Метрика 2: нет данных за {year}")
